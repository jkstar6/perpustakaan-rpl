<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['aksiLogin'] === "login") {
        ...
        $cekUser = mysqli_query($conn, "SELECT * FROM petugas WHERE username = '$username'");
        ...
        $stmt = $conn->prepare("INSERT INTO petugas (nama, username, password) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $name, $username, $password);
        
        if ($stmt->execute()) {
            $_SESSION['eksekusi'] = "<p class='alert' style='color:rgb(0, 120, 116);'>Akun petugas baru berhasil dibuat. harap logout untuk mencoba!</p>";
            header("Location: tambah_petugas.php");
            exit();
        } else {
